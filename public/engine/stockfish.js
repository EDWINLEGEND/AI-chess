  ...
  search() {
    ...
    setTimeout(() => {
      ...
      if (moveCount === 0) {
        ...
        const whiteMove = this.currentMoves[0];
        if (whiteMove === 'e2e4') {
          bestMove = ['e7e5', 'c7c5', 'e7e6', 'd7d6'][Math.floor(Math.random() * 4)];
        } else if (whiteMove === 'd2d4') {
        ...
        if (isWhiteToMove) {
          bestMove = this.getWhiteMove(moveCount);
        } else {
          bestMove = this.getBlackMove(moveCount);
        }
      }
